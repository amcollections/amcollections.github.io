...
$(function() {
  ...
  $('#collections').on('change', '', function (event) {
      ...
      if (filename !== "") {
        ...
        $("#browse").html('<a href="https://www.aucklandmuseum.com/collections">Browse this collection</a>');      
        $.getJSON(filename, function(data) {
            var htmlString = "";      
            $.each(data.images, function(key, val) {
               var rowString = '<a href="' + val.id + '" target="_blank">';
               rowString += '<svg class="img-fluid" height="20" width="120">';
